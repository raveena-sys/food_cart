...
class CategoryRepository
{
    ...
    public function getList($request)
    {
        try {
            ...
            $query = $this->category->select('category.*')->where('category.status', '!=', 'deleted');
            ...
            $category = $query->orderby('category.id','desc')->get();
            $tableResult = Datatables::of($category)->addIndexColumn()
                ...
                ->addColumn('action', function ($row) {
                    if(Auth::user()->user_type == 'store')
                    {
                        $viewUrl = url('store/manage-category/view/'.$row->id);
                    }else{
                        $viewUrl = url('admin/manage-category/view/'. $row->id);
                    }
    ...
    {
        try {
            $query = $this->category->select('category.*')->where('category.id', $id);
            if(Auth::user()->user_type == 'store'){
    ...
    */
    public static function delete($request)
    {
        try {
            ...
            $userData = Category::where(['id' => $request->id])->select('id', 'name', 'description', 'status', 'store_id')->first();
            if (!empty($userData)) {
                if(Auth::user()->user_type =='store'){
