      ...
      <div class="card listGrid">
        <div class="imgWrapper">
          ...
          <div class="img-footer">
            ...
            @if($val->topping_from == 'topping_pizza' || $val->topping_from == 'topping_dips' || $val->topping_from == 'topping_donair_shawarma_mediterranean' || $val->topping_from == 'topping_wing_flavour')
            <div class="price">
              <a class="customise_item btn btn-default customise" data-id="{{isset($val->id)?$val->id:0}}">Customise</a>
            </div>
            @endif
            @endif 
          </div>
        ...
        <div class="card-footer">
          ...
          ?>
            <div class="countWrap">
              <button type="button" id="sub" data-product_id="{{isset($val->id)?$val->id:''}}" class="sub sub_item" data-price="{{isset($v['price'])?$v['price']:0}}">-</button>
              <input class="count" type="text" id="item_count_{{isset($val->id)?$val->id:''}}" value="{{isset($quantity)?$quantity:1}}" min="1" max="100">
              <button type="button" id="add" class="add add_to_cart" data-product_id="{{isset($val->id)?$val->id:''}}" data-price="{{$price}}">+
                </button>
            </div>            
          @else
            <div class="leftSide">
              <a href = "javascript:void(0);" data-product_id="{{isset($val->id)?$val->id:''}}" data-price="{{isset($val->custom_price)?round($val->custom_price, 2):(isset($val->price)?round($val->price, 2):'')}}" class="btn btn-success add_to_cart">
                ADD TO CART
              </a>
            </div>  
          @endif
