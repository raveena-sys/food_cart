...
<script>
    (function categoryList() {
        let oTable = $('#category-listing').DataTable({
            ...
            ajax: {
                url: "{{url('admin/manage-pizza-size/list')}}",
                data: function(d) {
                    d.name = $('#name').val(),
                        d.status = $('#status').val(),
    ...
    function addCategory() {
        ...
        if ($('#add_category_form').valid()) {
            ...
            $.ajax({
                type: "POST",
                url: "{{url('admin/manage-pizza-size/add')}}",
                data: formData,
    ...
    function changeStatus(id) {
        bootbox.confirm('Are you sure you want to change the status ?', function(result) {
            ...
            if (result) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{url('admin/manage-pizza-size/change-status')}}",
                    data: {
                        id: id,
                        status: status,
                        _token: "{{csrf_token()}}"
    ...
    function ediCategory(id) {
        var url = "{{url('admin/manage-pizza-size/detail')}}";
        $.ajax({
            type: "GET",
            url: url + '/' + id,
            success: function(response) {
    ...
    function deleteCategory(id) {
        bootbox.confirm('Are you sure you want to delete ?', function(result) {
            if (result) {
                var url = "{{url('admin/manage-pizza-size/delete')}}";
                $.ajax({
                    type: "DELETE",
                    url: url + '/' + id,
                    data: {
