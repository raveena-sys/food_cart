        ...
        <div class="modal-content">
            ...
            <div class="modal-body field-padd">
                <form id="add_category_form" method="POST" class="needs-validation" novalidate autocomplete="false" action="{{URL::To('admin/manage-pizza-size/add')}}">
                    ...
                    <div class="form-group">
                        <label>Price</label>
                        <input class="form-control" name="price" type="text" placeholder="Name">
                    </div>
                    <div class="form-group">
...
<script>
    (function categoryList() {
        let oTable = $('#category-listing').DataTable({
            ...
            ajax: {
                url: "{{url('admin/manage-topping-wing-flavour/list')}}",
                data: function(d) {
                    d.name = $('#name').val(),
                        d.status = $('#status').val(),
                        d.price = $('#price').val(),
    ...
    function addCategory() {
        ...
        if ($('#add_category_form').valid()) {
            ...
            $.ajax({
                type: "POST",
                url: "{{url('admin/manage-topping-wing-flavour/add')}}",
                data: formData,
    ...
    function changeStatus(id) {
        bootbox.confirm('Are you sure you want to change the status ?', function(result) {
            ...
            if (result) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{url('admin/manage-topping-wing-flavour/change-status')}}",
                    data: {
                        id: id,
                        status: status,
                        _token: "{{csrf_token()}}"
    ...
    function ediCategory(id) {
        var url = "{{url('admin/manage-topping-wing-flavour/detail')}}";
        $.ajax({
            type: "GET",
            url: url + '/' + id,
            success: function(response) {
    ...
    function deleteCategory(id) {
        bootbox.confirm('Are you sure you want to delete ?', function(result) {
            if (result) {
                var url = "{{url('admin/manage-topping-wing-flavour/delete')}}";
                $.ajax({
                    type: "DELETE",
                    url: url + '/' + id,
                    data: {
