<div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title font-libre-bold w-100 text-center">Edit Pizza Crust</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <i class="icon-clear"></i>
            </button>
        </div>
        <div class="modal-body field-padd" id="edit-form">
            <form id="edit_category_form" action="{{url('admin/manage-pizza-crust/update')}}" method="POST" class="needs-validation" novalidate autocomplete="false">
                {{csrf_field()}}
                <input type="hidden" name="id" value="{{$detail->id}}">
                <input type="hidden" name="storeid" value="@if($detail->store_id){{$detail->store_id}}@endif">
                <!-- {{$detail}} -->
...
<script>
    function updateCategory() {
        ...
        if ($('#edit_category_form').valid()) {
            ...
            $.ajax({
                type: "POST",
                url: "{{url('admin/manage-pizza-crust/update')}}",
                data: formData,
