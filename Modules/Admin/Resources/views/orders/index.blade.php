    ...
    <div class="container-fluid">
        ...
        <div class="page-title-row filter-page-btn">
          ...
          <!--   <div class="right-side">
                ...
                <a href="{{ URL::To('admin/orders/add') }}" class="btn btn-danger ripple-effect text-uppercase">Add Product</a>
            </div> -->
        </div>
        <!-- Filter Start -->
...
<script>
    (function categoryList() {
        let oTable = $('#category-listing').DataTable({
            ...
            ajax: {
                url: "{{url('admin/orders/list-data')}}",
                data: function(d) {
                    d.name = $('#name').val(),
                        d.status = $('#status').val()
                    //,d.type = 'appraiser'
    ...
    function deleteOrder(id) {
        bootbox.confirm('Are you sure you want to delete ?', function(result) {
            if (result) {
                var url = "{{url('admin/orders/delete')}}";
                $.ajax({
                    type: "DELETE",
                    url: url + '/' + id,
                    data: {
