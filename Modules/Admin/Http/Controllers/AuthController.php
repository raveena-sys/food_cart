...
use AuthenticatesUsers;
    ...
    public function index() {
        ...
        return view('admin::profile-setting.index',['adminDetail'=>$adminDetail]);
    }
    ...
    public function forgotPassord(Request $request) {
        $post = $request->all();
        try {
            $user = User::where(['email'=> $post['email'],'user_type'=>'admin'])->first();
            if (!empty($user)) {
    ...
    public function resetPassword($token) {
        ...
        $user = User::where(['user_type'=>'admin','verify_token'=>$token])->first();
        if (!empty($user)) {
