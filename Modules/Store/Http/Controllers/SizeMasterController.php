...
class SizeMasterController extends Controller
{
    ...
    {
        return view('store::manage-master.size-master.index');
    }
    ...
    public function getSizeMasterDetails($id)
    {
        try {
            ...
            return View::make('store::manage-master.size-master.edit', ['detail' => $data])->render();
        } catch (\Exception $e) {
            return Response::json(['success' => false, 'message' => $e->getMessage()]);
        }
    }
    ...
    {
        $user = $this->SizeMasterRepository->getDetail($id);
        return view('store::manage-master.size-master.view', ['category' => $user]);
    }
