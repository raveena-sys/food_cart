    ...
    <div class="container-fluid">
        ...
        <div class="page-title-row filter-page-btn">
            <div class="left-side">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Pizza Crust</li>
                    </ol>
                </nav>
            ...
            <div class="right-side">
                ...
                <a href="javascript:void(0);" onclick="showAddCategory()" class="btn btn-danger ripple-effect text-uppercase">Add Pizza Crust</a>
                @if(Auth::user()->user_type == 'store')
                <a href="javascript:void(0);" onclick="showSelectCrusts()" class="btn btn-danger ripple-effect text-uppercase">Select Pizza Crust</a>
                ...
                <table class="table" id="category-listing">
                    <thead>
                        <tr>
                            ...
                            <th><span class="sorting">Price ($)</span></th>
                            <th> Description </th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title font-libre-bold w-100 text-center">Add Pizza Crust</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i class="icon-clear"></i>
                </button>
            </div>
            <div class="modal-body field-padd">
                <form id="add_category_form" method="POST" class="needs-validation" novalidate autocomplete="false" action="{{URL::To('store/manage-pizza-crust/add')}}">
                    {{csrf_field()}}
                    ...
                    <div class="form-group">
                    <label>Price</label>
                        <input class="form-control" name="price" type="text" placeholder="Price" maxlength="250" value="">
                    </div>
                    <div class="form-group">
                ...
                {!! JsValidator::formRequest('Modules\Admin\Http\Requests\AddPizzaCrustRequest','#add_category_form')
                !!}
            </div>
        </div>
    </div>
...
<div class="modal modal-effect" id="customPriceModal" tabindex="-1" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="" aria-hidden="true">
       <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title font-libre-bold w-100 text-center product_price">Select Pizza Crust to get in your Store</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i class="icon-clear"></i>
                </button>
            </div>
            <div class="modal-body field-padd">
                <form id="add_category_form" method="POST" class="needs-validation" novalidate autocomplete="false" action="{{URL::To('store/manage-category/add')}}">
                    ...
                    <label class="label_crust_error help-block error-help-block form-text text-danger"></label>                  
                    <div class="form-group text-center mb-0">
                        <button id="btnAdd" class="btn btn-danger ripple-effect text-uppercase min-w130" onClick="addCrustSelect()" type="button">Select<span id="btnAddLoader" class="spinner-border spinner-border-sm" style="display: none;"></span></button>
...
<script>
    (function categoryList() {
        let oTable = $('#category-listing').DataTable({
            ...
            ajax: {
                url: "{{url('store/manage-pizza-crust/list')}}",
                data: function(d) {
                    d.name = $('#name').val(),
                        d.status = $('#status').val(),
                        d.type = 'appraiser'
    ...
    function addCategory() {
        ...
        if ($('#add_category_form').valid()) {
            ...
            $.ajax({
                type: "POST",
                url: "{{url('store/manage-pizza-crust/add')}}",
                data: formData,
    ...
    function changeStatus(id) {
        bootbox.confirm('Are you sure you want to change the status ?', function(result) {
            ...
            if (result) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{url('store/manage-pizza-crust/change-status')}}",
                    data: {
                        id: id,
                        status: status,
                        _token: "{{csrf_token()}}"
    ...
    function ediCategory(id) {
        var url = "{{url('store/manage-pizza-crust/detail')}}";
        $.ajax({
            type: "GET",
            url: url + '/' + id,
            success: function(response) {
    ...
    function deleteCategory(id) {
        bootbox.confirm('Are you sure you want to delete ?', function(result) {
            if (result) {
                var url = "{{url('store/manage-pizza-crust/delete')}}";
                $.ajax({
                    type: "DELETE",
                    url: url + '/' + id,
                    data: {
    ...
    function showSelectCrusts(id=''){
        $.ajax({
            url:"{{url('store/manage-pizza-crust/select')}}",
            type:'get',
            data:{id:id},
            success:function(response){
                $('.crust_price').empty();
                $('.crust_price').html('Select Pizza Crust to get in your Store');
                $("#customPriceModal").modal('show');
                if($('.crust_id:last').val()!= ''){
                    $('.crust_list').append(response);
                }
            }
        })
    ...
    $(document).on('change', '.crust_id', function(){
        if($(this).val()!=''){
            var selected = $('.crust_id');
            selectedIDS = '';            
            ...
            $(this).parent().next().find('.crust_price').val(price);
            showSelectCrusts(selectedIDS);
        }else{
            $(this).parent().next().find('.crust_price').val('0');
        }
    })


    function addCrustSelect(){
        var selected = $('.crust_id');
        selectedIDArray = [];        
        $.each(selected, function(obj, val){
            if($(this).val() != ''){ 
                selectedIDArray.push({'id':$("option:selected", this).val(),'price':$(this).parent().next().find('.crust_price').val()});
            }            
        });
        if(selectedIDArray.length>0){
           $('.label_crust_error').empty(); 
            $.ajax({
            url:"{{url('store/manage-pizza-crust/addselection')}}",
            type:'post',
            data:{
                '_token': "{{csrf_token()}}",
                id:selectedIDArray
            ...
            success:function(response){
                ...
                $('.crust_list').find('.row').not(":last").remove();
                $('#category-listing').DataTable().ajax.reload();
            },
            error: function() {
                Command: toastr['success']('Something went wrong.');
           ...
           $('.label_crust_error').html('This field is required'); 
