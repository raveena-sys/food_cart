    ...
    <div class="container-fluid">
        ...
        <div class="page-title-row filter-page-btn">
            <div class="left-side">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Pizza Size</li>
                    </ol>
                </nav>
            ...
            <div class="right-side">
                ...
                <a href="javascript:void(0);" onclick="showAddCategory()" class="btn btn-danger ripple-effect text-uppercase">Add Pizza Size</a>
                @if(Auth::user()->user_type == 'store')
                <a href="javascript:void(0);" onclick="showSelectSize()" class="btn btn-danger ripple-effect text-uppercase">Select Pizza Size</a>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title font-libre-bold w-100 text-center">Add Pizza Size</h4>
            ...
            <div class="modal-body field-padd">
                <form id="add_category_form" method="POST" class="needs-validation" novalidate autocomplete="false" action="{{URL::To('store/manage-pizza-size/add')}}">
                    ...
                    <div class="form-group">
                        ...
                        @endphp
                        <select class="form-control selectpicker" name="size_master_id" id="comanyList" title="Select Size" data-size="4">
                    ...
                    <div class="form-group has-error">
                        <label>Price</label>
                        <input class="form-control" name="price" type="text" placeholder="Price" maxlength="250" >
                    </div>
                    <div class="form-group">
                ...
                {!! JsValidator::formRequest('Modules\Admin\Http\Requests\AddPizzaSizeRequest','#add_category_form')
                !!}
            </div>
        </div>
    </div>
...
<div class="modal modal-effect" id="customPriceModal" tabindex="-1" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="" aria-hidden="true">
       <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title font-libre-bold w-100 text-center product_price">Select Pizza Size to get in your Store</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i class="icon-clear"></i>
                </button>
            </div>
            <div class="modal-body field-padd">
                <form id="add_category_form" method="POST" class="needs-validation" novalidate autocomplete="false" action="{{URL::To('store/manage-category/add')}}">
                    ...
                    <label class="label_size_error help-block error-help-block form-text text-danger"></label>                  
                    <div class="form-group text-center mb-0">
                        <button id="btnAdd" class="btn btn-danger ripple-effect text-uppercase min-w130" onClick="addSizeSelect()" type="button">Select<span id="btnAddLoader" class="spinner-border spinner-border-sm" style="display: none;"></span></button>
...
<script>
    (function categoryList() {
        let oTable = $('#category-listing').DataTable({
            ...
            ajax: {
                url: "{{url('store/manage-pizza-size/list')}}",
                data: function(d) {
                    d.name = $('#name').val(),
                        d.status = $('#status').val(),
    ...
    function addCategory() {
        ...
        if ($('#add_category_form').valid()) {
            ...
            $.ajax({
                type: "POST",
                url: "{{url('store/manage-pizza-size/add')}}",
                data: formData,
    ...
    function changeStatus(id) {
        bootbox.confirm('Are you sure you want to change the status ?', function(result) {
            ...
            if (result) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{url('store/manage-pizza-size/change-status')}}",
                    data: {
                        id: id,
                        status: status,
                        _token: "{{csrf_token()}}"
    ...
    function ediCategory(id) {
        var url = "{{url('store/manage-pizza-size/detail')}}";
        $.ajax({
            type: "GET",
            url: url + '/' + id,
            success: function(response) {
    ...
    function deleteCategory(id) {
        bootbox.confirm('Are you sure you want to delete ?', function(result) {
            if (result) {
                var url = "{{url('store/manage-pizza-size/delete')}}";
                $.ajax({
                    type: "DELETE",
                    url: url + '/' + id,
                    data: {
    ...
    function showSelectSize(id=''){
        $.ajax({
            url:"{{url('store/manage-pizza-size/select')}}",
            type:'get',
            data:{id:id},
            success:function(response){
                $('.size_price').empty();
                $('.size_price').html('Select Pizza Size to get in your Store');
                $("#customPriceModal").modal('show');
                if($('.size_id:last').val()!= ''){
                    $('.size_list').append(response);
                }
            }
        })
    ...
    $(document).on('change', '.size_id', function(){
        if($(this).val()!=''){
            ...
            $(this).parent().next().find('.size_price').val(price);
            showSelectSize(selectedIDS);
        }else{
            $(this).parent().next().find('.size_price').val('0');
        }
    })
    ...
    function addSizeSelect(){
        ...
        $.each(selected, function(obj, val){
            if($(this).val() != ''){ 
                selectedIDArray.push({'id':$("option:selected", this).val(),'price':$(this).parent().next().find('.size_price').val()});
            }            
        });
        if(selectedIDArray.length>0){
           $('.label_size_error').empty(); 
            $.ajax({
            url:"{{url('store/manage-pizza-size/addselection')}}",
            type:'post',
            data:{
                '_token': "{{csrf_token()}}",
                id:selectedIDArray
            ...
            success:function(response){
                ...
                $('.size_list').find('.row').not(":last").remove();
                $('#category-listing').DataTable().ajax.reload();
            },
            error: function() {
                Command: toastr['success']('Something went wrong.');
           ...
           $('.label_size_error').html('This field is required'); 
