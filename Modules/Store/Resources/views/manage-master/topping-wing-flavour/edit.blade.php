<div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title font-libre-bold w-100 text-center">Edit Topping Wing Flavour</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <i class="icon-clear"></i>
            </button>
        </div>
        <div class="modal-body field-padd" id="edit-form">
            <form id="edit_category_form" action="{{url('store/manage-topping-wing-flavour/update')}}" method="POST" class="needs-validation" novalidate autocomplete="false">
                {{csrf_field()}}
            ...
            {!! JsValidator::formRequest('Modules\Admin\Http\Requests\EditToppingWingFlavourRequest','#edit_category_form') !!}
        </div>
    </div>
</div>
...
<script>
    function updateCategory() {
        ...
        if ($('#edit_category_form').valid()) {
            ...
            $.ajax({
                type: "POST",
                url: "{{url('store/manage-topping-wing-flavour/update')}}",
                data: formData,
