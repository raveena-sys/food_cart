...
@section('content')
<?php $current = 'Topping Donair Shawarma Mediterranean'; ?>
    ...
    <div class="container-fluid">
        ...
        <div class="page-title-row filter-page-btn">
            <div class="left-side">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Topping Donair Shawarma Mediterranean</li>
                    </ol>
                </nav>
            ...
            <div class="right-side">
                ...
                <a href="javascript:void(0);" onclick="showAddCategory()" class="btn btn-danger ripple-effect text-uppercase">Add Topping </a>
                @if(Auth::user()->user_type == 'store')
                <a href="javascript:void(0);" onclick="showSelectToppingDon()" class="btn btn-danger ripple-effect text-uppercase">Select Topping</a>
                @endif
            </div>
        </div>
        <!-- Filter Start -->
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title font-libre-bold w-100 text-center">Add Topping Donair Shawarma Mediterranean</h4>
            ...
            <div class="modal-body field-padd">
                ...
                {!! JsValidator::formRequest('Modules\Admin\Http\Requests\AddToppingDonairShawarmaMediterraneanRequest','#add_category_form')
                !!}
            </div>
        </div>
    </div>
...
<div class="modal modal-effect" id="customPriceModal" tabindex="-1" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="" aria-hidden="true">
       <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title font-libre-bold w-100 text-center top_don_price">Select Topping Pizza to get in your Store</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i class="icon-clear"></i>
                </button>
            </div>
            <div class="modal-body field-padd">
                <form id="add_category_form" method="POST" class="needs-validation" novalidate autocomplete="false" action="{{URL::To('store/manage-category/add')}}">
                    ...
                    <div class="row top_don_list">
                        <div class="form-group col-md-8">
                            <label>Topping Pizza</label>
                        </div>
                        <div class="form-group col-md-4">
                            <label>Price</label> 
                        </div>
                    </div> 
                    <label class="label_top_don_error help-block error-help-block form-text text-danger"></label>                  
                    <div class="form-group text-center mb-0">
                        <button id="btnAdd" class="btn btn-danger ripple-effect text-uppercase min-w130" onClick="addTopDonSelect()" type="button">Select<span id="btnAddLoader" class="spinner-border spinner-border-sm" style="display: none;"></span></button>
...
<script>
    (function categoryList() {
        let oTable = $('#category-listing').DataTable({
            ...
            ajax: {
                url: "{{url('store/manage-topping-donair-shawarma-mediterranean/list')}}",
                data: function(d) {
                    d.name = $('#name').val(),
                        d.status = $('#status').val(),
                        d.price = $('#price').val(),
    ...
    function addCategory() {
        ...
        if ($('#add_category_form').valid()) {
            ...
            $.ajax({
                type: "POST",
                url: "{{url('store/manage-topping-donair-shawarma-mediterranean/add')}}",
                data: formData,
    ...
    function changeStatus(id) {
        bootbox.confirm('Are you sure you want to change the status ?', function(result) {
            ...
            if (result) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{url('store/manage-topping-donair-shawarma-mediterranean/change-status')}}",
                    data: {
                        id: id,
                        status: status,
                        _token: "{{csrf_token()}}"
    ...
    function ediCategory(id) {
        var url = "{{url('store/manage-topping-donair-shawarma-mediterranean/detail')}}";
        $.ajax({
            type: "GET",
            url: url + '/' + id,
            success: function(response) {
    ...
    function deleteCategory(id) {
        bootbox.confirm('Are you sure you want to delete ?', function(result) {
            if (result) {
                var url = "{{url('store/manage-topping-donair-shawarma-mediterranean/delete')}}";
                $.ajax({
                    type: "DELETE",
                    url: url + '/' + id,
                    data: {
    ...
    function showSelectToppingDon(id=''){
        $.ajax({
            url:"{{url('store/manage-topping-donair-shawarma-mediterranean/select')}}",
            type:'get',
            data:{id:id},
            success:function(response){
                $('.top_don_price').empty();
                $('.top_don_price').html('Select Topping Pizza to get in your Store');
                $("#customPriceModal").modal('show');
                if($('.top_don_id:last').val()!= ''){
                    $('.top_don_list').append(response);
                }
            }
        })
    ...
    $(document).on('change', '.top_don_id', function(){
        if($(this).val()!=''){
            var selected = $('.top_don_id');
            selectedIDS = '';            
            ...
            $(this).parent().next().find('.top_don_price').val(price);
            showSelectToppingDon(selectedIDS);
        }else{
            $(this).parent().next().find('.top_don_price').val('0');
        }
    })


    function addTopDonSelect(){
        var selected = $('.top_don_id');
        selectedIDArray = [];        
        $.each(selected, function(obj, val){
            if($(this).val() != ''){ 
                selectedIDArray.push({'id':$("option:selected", this).val(),'price':$(this).parent().next().find('.top_don_price').val()});
            }            
        });
        if(selectedIDArray.length>0){
           $('.label_top_don_error').empty(); 
            $.ajax({
            url:"{{url('store/manage-topping-donair-shawarma-mediterranean/addselection')}}",
            type:'post',
            data:{
                '_token': "{{csrf_token()}}",
                id:selectedIDArray
            ...
            success:function(response){
                ...
                $('.top_don_list').find('.row').not(":last").remove();
                $('#category-listing').DataTable().ajax.reload();
            },
            error: function() {
                Command: toastr['success']('Something went wrong.');
           ...
           $('.label_top_don_error').html('This field is required'); 
