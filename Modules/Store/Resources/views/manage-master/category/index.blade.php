    ...
    <div class="container-fluid">
        ...
        <div class="page-title-row filter-page-btn">
            ...
            <div class="right-side">
                ...
                <a href="javascript:void(0);" onclick="showAddCategory()" class="btn btn-danger ripple-effect text-uppercase">Add Category</a>
                @if(Auth::user()->user_type == 'store')
                <a href="javascript:void(0);" onclick="showSelectCategory()" class="btn btn-danger ripple-effect text-uppercase">Select Category</a>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title font-libre-bold w-100 text-center">Add Category</h4>
...
<div class="modal modal-effect" id="customPriceModal" tabindex="-1" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="" aria-hidden="true">
       <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title font-libre-bold w-100 text-center product_price">Select Category to get in your Store</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i class="icon-clear"></i>
                </button>
            </div>
            <div class="modal-body field-padd">
                <form id="add_category_form" method="POST" class="needs-validation" novalidate autocomplete="false" action="{{URL::To('store/manage-category/add')}}">
                    ...
                    <label class="label_category_error help-block error-help-block form-text text-danger"></label>                  
                    <div class="form-group text-center mb-0">
                        <button id="btnAdd" class="btn btn-danger ripple-effect text-uppercase min-w130" onClick="addCategorySelect()" type="button">Select<span id="btnAddLoader" class="spinner-border spinner-border-sm" style="display: none;"></span></button>
...
<script>
    (function categoryList() {
        let oTable = $('#category-listing').DataTable({
            ...
            ajax: {
                url: "{{url('store/manage-category/list')}}",
                data: function(d) {
                    d.name = $('#name').val(),
                        d.status = $('#status').val(),
                        d.type = 'appraiser'
    ...
    function addCategory() {
        ...
        if ($('#add_category_form').valid()) {
            ...
            $.ajax({
                type: "POST",
                url: "{{url('store/manage-category/add')}}",
                data: formData,
    ...
    function deleteCategory(id) {
        bootbox.confirm('Are you sure you want to delete ?', function(result) {
            if (result) {
                var url = "{{url('store/manage-category/delete')}}";
                $.ajax({
                    type: "DELETE",
                    url: url + '/' + id,
                    data: {
    ...
    function showSelectCategory(id=''){
        $.ajax({
            url:"{{url('store/manage-category/select')}}",
            type:'get',
            data:{id:id},
            success:function(response){
                $('.category_price').empty();
                $('.category_price').html('Select Category to get in your Store');
                $("#customPriceModal").modal('show');
                if($('.category_id:last').val()!= ''){
    ...
    $(document).on('change', '.category_id', function(){
        if($(this).val()!=''){
            ...
            showSelectCategory(selectedIDS);
        }
    })
    ...
    function addCategorySelect(){
        ...
        $.each(selected, function(obj, val){
            if($(this).val() != ''){ 
                selectedIDArray.push({'id':$("option:selected", this).val(),'price':$(this).parent().next().find('.category_price').val()});
            }            
        });
        if(selectedIDArray.length>0){
           $('.label_category_error').empty(); 
            $.ajax({
            url:"{{url('store/manage-category/addselection')}}",
            type:'post',
            data:{
                '_token': "{{csrf_token()}}",
                id:selectedIDArray
            ...
            success:function(response){
                ...
                $('.category_list').find('.row').not(":last").remove();
                $('#category-listing').DataTable().ajax.reload();
            },
            error: function() {
                Command: toastr['success']('Something went wrong.');
           ...
           $('.label_category_error').html('This field is required'); 
