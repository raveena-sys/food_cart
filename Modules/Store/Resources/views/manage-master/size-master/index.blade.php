    ...
    <div class="container-fluid">
        ...
        <div class="page-title-row filter-page-btn">
            ...
            <div class="right-side">
                ...
                <a href="javascript:void(0);" onclick="showAddCategory()" class="btn btn-danger ripple-effect text-uppercase">Add Size Master</a>
            </div>
        </div>
        <!-- Filter Start -->
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title font-libre-bold w-100 text-center">Add Size Master</h4>
            ...
            <div class="modal-body field-padd">
                <form id="add_category_form" method="POST" class="needs-validation" novalidate autocomplete="false" action="{{URL::To('store/manage-pizza-size/add')}}">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" name="short_name" type="text" placeholder="Name" maxlength="250">
                    </div>
                    <div class="form-group">
                        <label>Price</label>
                        <input class="form-control" name="value" type="text" placeholder="Price" maxlength="20">
                    </div>
                    <div class="form-group">
                ...
                {!! JsValidator::formRequest('Modules\Admin\Http\Requests\AddSizeMasterRequest','#add_category_form')
                !!}
            </div>
        </div>
    </div>
...
<script>
    (function categoryList() {
        let oTable = $('#category-listing').DataTable({
            ...
            ajax: {
                url: "{{url('store/manage-size-master/list')}}",
                data: function(d) {
                    d.name = $('#name').val(),
                        d.status = $('#status').val(),
                        d.type = 'appraiser'
    ...
    function addCategory() {
        ...
        if ($('#add_category_form').valid()) {
            ...
            $.ajax({
                type: "POST",
                url: "{{url('store/manage-size-master/add')}}",
                data: formData,
    ...
    function changeStatus(id) {
        bootbox.confirm('Are you sure you want to change the status ?', function(result) {
            ...
            if (result) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{url('store/manage-size-master/change-status')}}",
                    data: {
                        id: id,
                        status: status,
                        _token: "{{csrf_token()}}"
    ...
    function ediCategory(id) {
        var url = "{{url('store/manage-size-master/detail')}}";
        $.ajax({
            type: "GET",
            url: url + '/' + id,
            success: function(response) {
    ...
    function deleteCategory(id) {
        bootbox.confirm('Are you sure you want to delete ?', function(result) {
            if (result) {
                var url = "{{url('store/manage-size-master/delete')}}";
                $.ajax({
                    type: "DELETE",
                    url: url + '/' + id,
                    data: {
